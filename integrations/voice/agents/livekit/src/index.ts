    ...
    try {
      ...
      if (options.telephonyCredentials?.apiKey && options.telephonyCredentials?.apiToken && !options.telephonyCredentials?.subdomain && !options.telephonyCredentials?.accountSid) {
        ...
        try {
          ...
          const answerUrl = `${publicUrl}/api/marketing/voice/webhooks/vobiz-answer?campaignId=${options.campaignId || 'direct_test'}&firstMessage=${encodeURIComponent(message)}&scriptPrompt=${encodeURIComponent(options.scriptPrompt || '')}&agent=LIVEKIT`;
          ...
          if (res.status >= 200 && res.status < 300) {
            ...
            return {
              success: true,
              providerCallId: data.callId || data.call_uuid || `vobiz_lk_${Date.now()}`,
            };
          }
        } catch {
      ...
      if (options.telephonyCredentials?.apiKey && options.telephonyCredentials?.apiToken && options.telephonyCredentials?.accountSid && options.telephonyCredentials?.subdomain) {
        ...
        try {
          ...
          if (res.status >= 200 && res.status < 300) {
            ...
            return {
              success: true,
              providerCallId: data?.Call?.Sid || data?.sid || `exotel_lk_${Date.now()}`,
            };
          }
        } catch {
          // fallback to LiveKit API
      ...
      const rawServer = credentials?.serverUrl || this.serverUrl || 'https://sumama-1kd20r85.livekit.cloud';
      ...
      // If LiveKit SIP gateway is not actively running, fallback gracefully with session ID
      return {
        success: true,
