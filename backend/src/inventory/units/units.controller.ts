...
export class UnitsController {
  ...
  async updateUnitStatus(@Param('unitId') unitId: string, @Body() data: any, @Req() req: any) {
  ...
  async updateProjectPossession(@Param('projectId') projectId: string, @Body() data: { status: any, timeline: any }) {
    return this.inventoryService.updateProjectPossession(projectId, data);
  }

  @Patch('towers/:towerId/possession')
  async updateTowerPossession(@Param('towerId') towerId: string, @Body() data: { status: any, timeline: any }) {
    return this.inventoryService.updateTowerPossession(towerId, data);
  }

  @Patch('units/:unitId/possession')
  async updateUnitPossession(@Param('unitId') unitId: string, @Body() data: { status: any, timeline: any }) {
