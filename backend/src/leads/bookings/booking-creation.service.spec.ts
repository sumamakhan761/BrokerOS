...
jest.mock('../../generated/prisma/client.js', () => ({ NotificationType: { BOOKING_CONFIRMED: 'BOOKING_CONFIRMED' }, PrismaClient: class {} }));
...
describe('BookingCreationService', () => {
  ...
  const mockPrisma = {
    ...
    unit: { findUnique: jest.fn(), update: jest.fn(), findFirst: jest.fn(), count: jest.fn() },
  ...
  it('should create booking and send notifications', async () => {
    ...
    mockPrisma.unit.findUnique.mockResolvedValue({ id: 'u-1', status: 'AVAILABLE', floor: { tower: { projectId: 'p-1' } } });
    mockPrisma.brokerProjectAssignment.findUnique.mockResolvedValue({ brokeragePercent: 2 });
    mockPrisma.booking.create.mockResolvedValue({ id: 'b-1' });
    mockPrisma.booking.findUnique.mockResolvedValue({ id: 'b-1', customer: { firstName: 'Test' }, unit: { floor: { tower: { projectId: 'p-1' } } } });
    mockPrisma.user.findMany.mockResolvedValue([]);
    mockPrisma.broker.findUnique.mockResolvedValue({ id: 'br-1' });
    mockPrisma.project.findUnique.mockResolvedValue({ id: 'p-1', name: 'Proj' });
