...
export function BrokerActionButtons({ broker, handleOpenMeetingModal, setIsFollowUpModalOpen, setIsNoteModalOpen }: BrokerActionButtonsProps) {
  return (
    <View className="flex-row flex-wrap gap-2 mb-4 justify-between mt-4">
      ...
      <TouchableOpacity onPress={() => setIsNoteModalOpen(true)} className="w-[31%] py-3 bg-indigo-600 rounded-xl flex-row justify-center items-center shadow-sm mt-2">
        <Feather name="edit-3" size={14} color="white" />
        <Text className="text-white font-bold ml-2 text-xs">Note</Text>
      </TouchableOpacity>

      <TouchableOpacity onPress={() => setIsFollowUpModalOpen(true)} className="w-[31%] py-3 bg-indigo-600 rounded-xl flex-row justify-center items-center shadow-sm mt-2">
        <Feather name="clock" size={14} color="white" />
        <Text className="text-white font-bold ml-2 text-xs">Follow-up</Text>
      </TouchableOpacity>

      {handleOpenMeetingModal && (
        <TouchableOpacity onPress={handleOpenMeetingModal} className="w-[31%] py-3 bg-indigo-600 rounded-xl flex-row justify-center items-center shadow-sm mt-2">
          <Feather name="users" size={14} color="white" />
          <Text className="text-white font-bold ml-2 text-xs">Meeting</Text>
        </TouchableOpacity>
      )}
    </View>
  );
