...
export default function SalesManagerLeadList() {
  ...
  useEffect(() => {
    fetchLeads();
  }, [status, scoreRange, filterFollowUpDate, siteVisitDate]);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <LeadListItem
            ...
            isSelected={false}
            toggleSelection={() => {}}
            onPressLead={(id) => router.push({ pathname: '/(dashboard)/sales-manager/lead-management/[id]', params: { id } } as any)}
            onLongPressLead={() => {}}
          />
        )}
