  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const res = await authClient.$fetch('/dashboard/director', { baseURL });
        setData(res);
      } catch (err: any) {
  ...
  return (
    <View className="flex-1 bg-[#f8fafc] p-6">
      <View className="items-center mb-8">
        <View className="w-16 h-16 rounded-2xl bg-rose-100 items-center justify-center mb-4">
          <Feather name="shield" size={32} color="#e11d48" />
        </View>
        <Text className="text-2xl font-bold text-gray-900">Director CRM</Text>
        <Text className="text-gray-500 text-center mt-2">Executive overview.</Text>
      </View>

      {loading && <ActivityIndicator size="large" color="#e11d48" />}
