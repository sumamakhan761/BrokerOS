...
export default function PostSalesInventoryIndex() {
  ...
  const loadProjects = async () => {
    try {
      ...
      // The POST_SALES role allows fetching all projects globally
      const { data, error } = await authClient.$fetch<any[]>('/api/inventory/projects', { baseURL: baseUrl });
  ...
  if (loading) {
    return (
      <View className="flex-1 items-center justify-center bg-slate-50">
        ...
        <Text className="text-slate-500 mt-4 font-medium">Loading global inventory...</Text>
      </View>
    );
  }
    ...
    <View className="flex-1 bg-slate-50">
      ...
      <View className="p-6 pt-12 bg-white border-b border-slate-200">
        <View className="flex-row items-center justify-between">
          <View>
            <Text className="text-2xl font-bold text-slate-900">Global Inventory</Text>
            <Text className="text-sm text-slate-500 mt-1">View all projects, towers, and units.</Text>
          </View>
        </View>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="py-20 items-center justify-center">
              ...
              <Text className="text-lg font-bold text-slate-900 mt-4">No Projects Found</Text>
              <Text className="text-slate-500 text-center mt-2 px-8">There are no projects available in the inventory yet.</Text>
            </View>
          }
          renderItem={({ item: project }) => (
            <TouchableOpacity 
              onPress={() => router.push(`/post-sales-manager/inventory/${project.id}` as any)}
