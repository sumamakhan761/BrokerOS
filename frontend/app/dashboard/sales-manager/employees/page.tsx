...
export default function SalesManagerEmployees() {
  ...
  async function handleSaveAnn() {
    ...
    try {
      if (editAnn) {
        await authClient.$fetch(`/api/dashboard/sales-manager/employees/announcements/${editAnn.id}`, {
  ...
  async function handleDeleteAnn(id: string) {
    ...
    await authClient.$fetch(`/api/dashboard/sales-manager/employees/announcements/${id}`, { method: "DELETE", baseURL: baseUrl });
    await load();
  }
  ...
  return (
    <div style={{ padding: "0 0 48px", display: "flex", flexDirection: "column", gap: 32 }}>
      ...
      {/* ── Modals ── */}
      <EmployeePageModals
        // Task logic is removed for Sales Manager
        showTaskModal={false}
        setShowTaskModal={() => { }}
        taskTarget={"0"}
        setTaskTarget={() => { }}
