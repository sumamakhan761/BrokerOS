...
export function RevenueCharts({ charts }: { charts: any }) {
  return (
    <section>
      <SectionHeader title="Revenue & Distribution" subtitle="Revenue breakdown by brokers and projects." />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-4">Project-wise Bookings</h3>
          <div className="h-64 w-full">
            {charts?.projectWiseBookings?.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={charts.projectWiseBookings} dataKey="units" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5}>
                    {charts.projectWiseBookings.map((_: any, index: number) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                  </Pie>
