...
import {
  SalesEmployeeGrid,
  SalesEmployee,
} from "./_components/SalesEmployeeGrid";
...
export default function SalesManagerEmployees() {
  ...
  async function load() {
    try {
      ...
      const [empRes, annRes] = await Promise.all([
        authClient.$fetch<SalesEmployee[]>(
          "/api/dashboard/sales-manager/employees",
          { baseURL: baseUrl }
        ),
        authClient.$fetch<Announcement[]>(
          "/api/dashboard/sales-manager/employees/announcements",
          { baseURL: baseUrl }
        ),
      ]);
    ...
    try {
      if (editAnn) {
        await authClient.$fetch(
          `/api/dashboard/sales-manager/employees/announcements/${editAnn.id}`,
          {
            method: "PATCH",
            baseURL: baseUrl,
  ...
  async function handleDeleteAnn(id: string) {
    if (
      ...
      return;
    await authClient.$fetch(
      `/api/dashboard/sales-manager/employees/announcements/${id}`,
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-8 animate-enter">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] tracking-tight flex items-center gap-2.5 m-0">
            ...
            <span>Sales Executives & Broadcast Feed</span>
          </h1>
          <p className="text-xs text-[var(--text-muted)] font-medium mt-0.5 m-0">
            Monitor real-time executive statuses, completed site visits & confirmed bookings
          </p>
        </div>
      </div>
      ...
      {/* Employee Cards Grid */}
      <SalesEmployeeGrid employees={employees} loading={loading} />
