...
import { ArrowRight, Trophy, CheckCircle2, FileText, ShieldCheck, Key, Home } from "lucide-react";
import { SectionHeader, fmt } from "./shared";

export function Insights({ data }: { data: any }) {
  return (
    <section>
      <SectionHeader title="Insights & Leaderboard" subtitle="Closing pipeline, top projects, and best performing brokers." />
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-900 mb-2">Closing Stage Pipeline (Cumulative)</h3>
          <p className="text-sm text-slate-500 mb-6">Each stage includes units that have progressed to it or beyond.</p>
          
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="text-slate-400 w-5 h-5" />
                <span className="font-semibold text-slate-700">Confirmed Bookings</span>
              </div>
              <span className="text-xl font-black text-slate-900">{data.funnel?.confirmed || 0}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-2xl border border-blue-100">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-blue-700">Documentation & Beyond</span>
              </div>
              <span className="text-xl font-black text-blue-900">{data.funnel?.documentation || 0}</span>
            </div>
            <div className="flex justify-center -my-2 z-10"><ArrowRight className="w-5 h-5 text-blue-300 rotate-90" /></div>
            
            <div className="flex items-center justify-between p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
              <div className="flex items-center gap-3">
                <ShieldCheck className="text-indigo-500 w-5 h-5" />
                <span className="font-semibold text-indigo-700">Loan/Agreement & Beyond</span>
              </div>
              <span className="text-xl font-black text-indigo-900">{data.funnel?.loanAgreement || 0}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-amber-50 rounded-2xl border border-amber-100">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-amber-700">Possession Pending & Beyond</span>
              </div>
              <span className="text-xl font-black text-amber-900">{data.funnel?.possession || 0}</span>
            </div>
            <div className="flex justify-center -my-2 z-10"><ArrowRight className="w-5 h-5 text-amber-300 rotate-90" /></div>
            
            <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
              <div className="flex items-center gap-3">
                <Home className="text-emerald-500 w-5 h-5" />
                <span className="font-semibold text-emerald-700">Handover Completed</span>
              </div>
              <span className="text-xl font-black text-emerald-900">{data.funnel?.handover || 0}</span>
            </div>
          </div>
        </div>
