    ...
    if (user?.roleId) {
      ...
      fetch(`${baseUrl}/roles`)
        ...
        .then((roles) => {
          ...
          if (
            role &&
            ["POST_SALES", "POST_SALES_MANAGER", "ADMIN"].includes(role.code)
          ) {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] tracking-tight flex items-center gap-2.5 m-0">
            ...
            <span>Completed Handovers & Archive</span>
          </h1>
          <p className="text-xs text-[var(--text-muted)] font-medium mt-0.5 m-0">
            Supervisor archive of customer files with concluded registration and unit handover
          </p>
        </div>
      </div>
