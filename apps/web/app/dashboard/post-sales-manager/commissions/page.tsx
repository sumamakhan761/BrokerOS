      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-[var(--text-primary)] tracking-tight flex items-center gap-2.5 m-0">
            ...
            <span>Manager Commission Supervision</span>
          </h1>
          <p className="text-xs text-[var(--text-muted)] font-medium mt-0.5 m-0">
            Audit inbound developer payouts and reconcile received builder brokerage
          </p>
        </div>
      </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-full border border-slate-200/60 w-fit">
          {["ALL", "PENDING", "RECEIVED"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3.5 py-1 rounded-full text-xs font-bold transition-all cursor-pointer ${filter === f
            ...
            >
              <div>
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider border ${comm.status === "RECEIVED"
