...
function LeadTableContent({ isManagerView }: { isManagerView?: boolean }) {
  ...
  const [scoreRange, setScoreRange] = useState(searchParams.get('scoreRange') || '');
  const [followUpDate, setFollowUpDate] = useState(initialFollowUpDate);
  const [siteVisitDate, setSiteVisitDate] = useState(searchParams.get('siteVisitDate') || '');
...
export function LeadTableClient({ isManagerView }: { isManagerView?: boolean }) {
  return (
