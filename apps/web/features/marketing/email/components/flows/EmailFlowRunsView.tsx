// ============================================================================
// BrokerOS — Email Flow Runs History View (WhatsApp Parity)
// ============================================================================
...
export function EmailFlowRunsView({ id }: { id: string }) {
  const router = useRouter();
  const [flowName, setFlowName] = useState('Email Flow');
  const [runs, setRuns] = useState<EmailFlowRunItem[]>([]);
    ...
    async function load() {
      try {
        ...
        const [fRes, rRes] = await Promise.all([
          fetch(`${baseUrl}/api/marketing/email/flows/${id}`, { credentials: 'include' }),
          fetch(`${baseUrl}/api/marketing/email/flows/${id}/runs`, { credentials: 'include' }),
        ]);

        if (fRes.ok) {
          const fData = await fRes.json();
          setFlowName(fData.name || 'Email Flow');
        }
  ...
  if (error) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-3">
        ...
        <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/marketing/email/flows/${id}`)}>
          Back to Flow
        </Button>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={() => router.push(`/dashboard/marketing/email/flows/${id}`)}
        ...
        <div>
          ...
          <p className="text-xs text-text-muted">Inbound Reply Triggers & Automated Execution Logs</p>
        </div>
      </div>
      ...
      {runs.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-border-default bg-bg-subtle/40 p-6 text-center">
          ...
          <p className="mt-1 text-xs text-text-muted max-w-md">
            When a prospect replies to a broadcast email matching this flow, their inbound message and automated execution logs will appear here.
          </p>
        </div>
      ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-text-primary truncate">
                      {run.inboundSubject || 'Inbound Prospect Reply'}
                    </p>
