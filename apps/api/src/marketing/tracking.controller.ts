...
export class TrackingController {
  ...
  async trackOpen(
    ...
    if (campaignId && recipientId) {
      ...
      await this.marketingService.recordOpenEvent(campaignId, recipientId, ip, userAgent);
    }
  ...
  async trackClick(
    ...
    if (campaignId && recipientId && targetUrl) {
      const ip = (req.headers['x-forwarded-for'] as string) || req.socket.remoteAddress;
      const userAgent = req.headers['user-agent'];
      await this.marketingService.recordClickEvent(campaignId, recipientId, targetUrl, ip, userAgent);
    }
  ...
  async unsubscribe(
    ...
    if (email) {
      await this.marketingService.recordUnsubscribe(email, campaignId);
    }
