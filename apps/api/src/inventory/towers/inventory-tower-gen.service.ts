  ...
  async generateTowerPrompt(projectId: string, prompt: string) {
    if (!this.groq) {
      throw new BadRequestException(
        'AI is not configured. Please set GROQ_API_KEY.',
      );
    }
        ...
        for (const f of towerData.floors || []) {
          const floorNumber =
            typeof f.floorNumber === 'number'
              ? f.floorNumber
              : parseInt(String(f.floorNumber).replace(/[^\d.-]/g, '')) || 0;
          ...
          for (const u of f.units || []) {
            ...
            const basePrice =
              parseFloat(String(u.basePrice || 0).replace(/[^\d.-]/g, '')) || 0;
            const carpetArea =
              parseFloat(String(u.carpetArea || 0).replace(/[^\d.-]/g, '')) ||
              0;
            let commissionPercentage: number | null = null;
            if (
              u.commissionPercentage !== undefined &&
              u.commissionPercentage !== null
            ) {
              commissionPercentage =
                parseFloat(
                  String(u.commissionPercentage).replace(/[^\d.-]/g, ''),
                ) || 0;
            }
