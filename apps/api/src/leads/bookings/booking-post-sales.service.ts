...
export class BookingPostSalesService {
  ...
  async saveHandover(bookingId: string, data: Record<string, any>) {
    ...
    try {
      ...
      if (
        cleanData.status === 'HANDED_OVER' ||
        cleanData.keysHandedOver === true
      ) {
        ...
        if (booking && booking.unitId) {
          ...
          if (unit && (unit.commissionAmount || unit.commissionPercentage)) {
            ...
            if (unit.commissionAmount) {
              ...
              amount =
                (Number(unit.commissionPercentage) / 100) *
                Number(booking.totalPayable);
            }

            if (amount > 0) {
              const existingComm =
                await this.prisma.inboundCommission.findFirst({
          ...
          if (cleanData.handoverById) {
            try {
              ...
              if (fullBookingForNotif) {
                const projectName =
                  fullBookingForNotif.unit?.floor?.tower?.project?.name ||
                  'Project';
                const unitNumber =
                  fullBookingForNotif.unit?.unitNumber || 'Unit';
                ...
                if (
                  ...
                  user?.role?.code === 'POST_SALES'
                ) {
                  const handoverCount =
                    await this.prisma.possessionHandover.count({
                      where: {
                        handoverById: cleanData.handoverById,
                        status: 'HANDED_OVER',
                      },
                    });
  ...
  async uploadPostSalesFile(
    ...
    if (type === 'loan') {
      if (fieldName === 'loanDocumentUrls') {
        const lc = await this.prisma.loanCase.findUnique({
          where: { bookingId },
        });
