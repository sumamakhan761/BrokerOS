...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleSendMessage(
    ...
    try {
      const message = await this.chatService.sendMessage(userId, data.roomId, data.content, data.attachment);
  ...
  handleTyping(@MessageBody() data: TypingIndicatorDto, @ConnectedSocket() client: Socket) {
    ...
    client.to(`room_${data.roomId}`).emit('user_typing', { userId, isTyping: data.isTyping, roomId: data.roomId });
  }
}
