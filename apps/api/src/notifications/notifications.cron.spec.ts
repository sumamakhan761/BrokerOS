...
describe('NotificationsCron', () => {
  ...
  it('should run handleMissedFollowUps', async () => {
    (prismaClient.followUp.findMany as jest.Mock).mockResolvedValue([{
      id: 'f-1', userId: 'u-1', status: 'SCHEDULED', user: { managerId: 'm-1', role: { code: 'PRE_SALES' } }
    }]);
    (prismaClient.user.findUnique as jest.Mock).mockResolvedValue({ role: { code: 'PRE_SALES_MANAGER' } });
  ...
  it('should run handleTomorrowSiteVisits', async () => {
    (prismaClient.siteVisit.findMany as jest.Mock).mockResolvedValue([{
      id: 's-1', salesExecId: 'u-1', scheduledDate: new Date(), lead: { firstName: 'Test' }
    }]);
  ...
  it('should run scheduleTodaySiteVisitArrivePush', async () => {
    (prismaClient.siteVisit.findMany as jest.Mock).mockResolvedValue([{
      id: 's-1', salesExecId: 'u-1', scheduledDate: new Date(Date.now() + 10 * 60000), lead: { firstName: 'Test' }, project: { name: 'Proj' }
    }]);
  ...
  it('should run sendMonthlyAnalyticsReports', async () => {
    (prismaClient.user.findMany as jest.Mock).mockResolvedValue([{ id: 'u-1', role: { code: 'PRE_SALES' } }]);
