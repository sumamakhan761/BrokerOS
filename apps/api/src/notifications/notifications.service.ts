...
export class NotificationsService {
  ...
  async checkDailyTaskCompletion(
    userId: string,
    taskType?: 'CALLS' | 'FOLLOW_UPS' | 'SITE_VISITS',
  ) {
    ...
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { role: true, expoPushToken: true },
    });
    if (
      !user ||
      !user.role ||
    ...
    const followUpsTarget = await prisma.followUp.count({
    ...
    if (role === 'PRE_SALES') {
      ...
      const callsTarget =
        assignment?.targetOverride ?? assignment?.task?.coldCallTarget ?? 100;
      progressString = `${coldCallCount} / ${callsTarget} Calls, ${followUpsDone} / ${followUpsTarget} Follow-ups`;
    } else if (role === 'SALES_EXECUTIVE') {
      const siteVisitsDone = await prisma.siteVisit.count({
        where: {
          salesExecId: userId,
          status: 'COMPLETED',
          completedAt: { gte: start, lt: end },
      ...
      const siteVisitsTarget = await prisma.siteVisit.count({
        where: { salesExecId: userId, scheduledDate: { gte: start, lt: end } },
      });
    ...
    try {
      ...
      this.logger.error(
        'Failed to send silent data push for daily progress',
        err,
      );
    }
  }
