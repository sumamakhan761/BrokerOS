...
describe('BrokerCommissionsService', () => {
  ...
  const mockPrisma = {
    ...
    brokerageRecord: { findMany: jest.fn(), findUnique: jest.fn(), update: jest.fn() },
  };

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [BrokerCommissionsService, { provide: PrismaService, useValue: mockPrisma }],
    }).compile();
  ...
  it('should complete commission', async () => {
    mockPrisma.brokerageRecord.findUnique.mockResolvedValue({ id: 'c-1', netPayable: 100 });
    mockPrisma.brokerageRecord.update.mockResolvedValue({ id: 'c-1', status: 'PAID' });
