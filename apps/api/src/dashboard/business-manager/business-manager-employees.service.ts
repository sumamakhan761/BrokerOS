  ...
  async getEmployees(period?: string) {
    ...
    const users = await this.prisma.user.findMany({
      ...
      include: {
        ...
        siteVisitsAsExec: dateFilter ? { where: { scheduledDate: dateFilter } } : true,
        salesExecBookings: dateFilter ? { where: { bookingDate: dateFilter, status: { not: 'CANCELLED' } } } : { where: { status: { not: 'CANCELLED' } } },
        closedBookings: dateFilter ? { where: { bookingDate: dateFilter, status: { not: 'CANCELLED' } } } : { where: { status: { not: 'CANCELLED' } } }
      }
    });
    ...
    const employees = users.map(u => {
      ...
      const bookingsCount = u.salesExecBookings.length + u.closedBookings.length;
      
      let bookingRevenue = 0;
      u.salesExecBookings.forEach(b => bookingRevenue += Number(b.agreedPrice) || 0);
      u.closedBookings.forEach(b => bookingRevenue += Number(b.agreedPrice) || 0);
    ...
    const salesEmployees = employees.filter(e => e.department.toUpperCase().includes('SALES') || e.role.toUpperCase().includes('SALES'));
    
    // Sort by calls to find bottom 20%
    const sortedByCalls = [...salesEmployees].sort((a, b) => a.metrics.calls - b.metrics.calls);
    const bottom20Count = Math.max(1, Math.floor(sortedByCalls.length * 0.2));
    const callLaggardIds = new Set(sortedByCalls.slice(0, bottom20Count).map(e => e.id));

    const laggards = salesEmployees.filter(e => {
