  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const res = await authClient.$fetch('/dashboard/sales', { baseURL });
        setData(res);
      } catch (err: any) {
    ...
    <View className="flex-1 bg-[#f8fafc] p-6">
      <View className="items-center mb-8">
        <View className="w-16 h-16 rounded-2xl bg-orange-100 items-center justify-center mb-4">
          <Feather name="briefcase" size={32} color="#ea580c" />
        </View>
        <Text className="text-2xl font-bold text-gray-900">Sales CRM</Text>
        <Text className="text-gray-500 text-center mt-2">Manage negotiations and closures.</Text>
      </View>
