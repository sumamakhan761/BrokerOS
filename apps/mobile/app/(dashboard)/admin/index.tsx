  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const res = await authClient.$fetch('/dashboard/admin', { baseURL });
        setData(res);
      } catch (err: any) {
    ...
    <View className="flex-1 bg-[#f8fafc] p-6">
      <View className="items-center mb-8">
        <View className="w-16 h-16 rounded-2xl bg-slate-200 items-center justify-center mb-4">
          <Feather name="settings" size={32} color="#475569" />
        </View>
        <Text className="text-2xl font-bold text-gray-900">Admin CRM</Text>
        <Text className="text-gray-500 text-center mt-2">System configurations.</Text>
      </View>

      {loading && <ActivityIndicator size="large" color="#475569" />}
