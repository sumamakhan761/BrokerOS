      ...
      {error ? (
        ...
        <FlatList
          ...
          renderItem={({ item: project }) => (
            <TouchableOpacity
              onPress={() => router.push(`/sales-manager/inventory/${project.id}` as any)}
            ...
            >
              ...
              <View className="mt-4 pt-4 border-t border-slate-100 flex-row justify-between items-center">
                ...
                <View className="flex-row items-center gap-3">
                  <TouchableOpacity 
                    onPress={() => setAssignmentModal({ isOpen: true, projectId: project.id, projectName: project.name })}
                    className="flex-row items-center bg-indigo-50 px-2 py-1 rounded-lg"
                  >
                    <Feather name="user-plus" size={14} color="#4f46e5" />
                    <Text className="text-indigo-600 text-xs font-bold ml-1">Assign</Text>
                  </TouchableOpacity>
                  <View className="flex-row items-center">
